...
<script>
import { getCollegeCourseStatus articleList} from '@/api/admin/article'
...
export default {
  ...
  methods: {
    ...
    getCollegeCourseStatus() {
      getCollegeCourseStatus().then(res => {
        if (res.meta.code === '200') {
          this.courseStateOptions = res.data
        }
      })
    },
    doSearch() {
